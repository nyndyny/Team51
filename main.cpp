...
int main() {
    ...
    while(getline(MyFile2, temp, ','))//getting each line on data and saving into separate variables
    {
    ...
    while (getline(MyFile3, temp2, ',')) {//getting each line on data and saving into separate variables
    ...
    while(getline(readDemoFile, s, ',')){//getting each line on data and saving into separate variables
    ...
    while (getline(timeSpan, temp3, ','))//getting each line on data and saving into separate variables
    {
    ...
    while(getline(readLoginFile, userName, ','))//getting each line on data and saving into separate variables
    {
    ...
    if (userResponse == "Yes" || userResponse == "yes"){//user has an account
        if(userLoginInfo.empty()){//if the login file is empty user is forced to create a login account(user does not have an account)
            ...
            cout << "Password: ";
            cin >> passWord;
            while(passWord.length() < 8){//if password is less than 8 characters
...
//showing the menu options to the user
    ...
    while(menuOption!=7)
    {
        ...
        {
            cout<<"User logged out successfully! Please re-login.\n";
    ...
    cout<<"Thank you for using Crime Fighters!\n";
    // Close the file
    MyFile.close();
}